// === WRITE SINE EQUATION FROM DESCRIPTION (ADVANCED) - All four transformations described in words. Student supplies a, b, h, k. ===
// === SET QUESTION TYPE TO: multipart ===

// === COMMON CONTROL ===

/* ---------- 1. Advanced (all 4 active in the description) ---------- */
$a_on = 1
$b_on = 1
$h_on = 1
$k_on = 1
      ...
      <table class="rubric-table">
        <tbody>
          ...
          <tr>
            ...
            <td>The general form is <b>y = a sin(b(x &minus; h)) + k</b>. Match each verbal property to the right parameter.</td>
          </tr>
          ...
          <tr>
            ...
            <td class="col-check-bot">' . ($k == 0 ? "No vertical shift means k = <b>0</b>." : ($k > 0 ? "Up " . $k . " unit(s) means k = <b>" . $k . "</b>." : "Down " . abs($k) . " unit(s) means k = <b>" . $k . "</b>.")) . '
              <div style="margin-top:10px;padding:0.6em 1em;background:#e8f5e9;border-left:4px solid #4CAF50;border-radius:0 6px 6px 0;">
                ...
                <b>Equation:</b> y = ' . ($a == 1 ? "" : ($a == -1 ? "-" : $a)) . 'sin(' . ($b == 1 && $h_deg == 0 ? "x" : ($b == 1 ? ($h_deg > 0 ? "x - " . $h_deg . "&deg;" : "x + " . abs($h_deg) . "&deg;") : ($h_deg == 0 ? $bdisp . "x" : $bdisp . "(x " . ($h_deg > 0 ? "- " . $h_deg : "+ " . abs($h_deg)) . "&deg;)"))) . ')' . ($k == 0 ? "" : ($k > 0 ? " + " . $k : " - " . abs($k))) . '
              </div>
            </td>
          </tr>
        </tbody>
