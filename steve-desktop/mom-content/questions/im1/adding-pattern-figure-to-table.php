...
$solutionguide = "<p>Read the counts off the figures: `$b` blocks at `t=0`, `$n[1]` at `t=1`, `$n[2]` at `t=2`.</p><p><b>The constant difference.</b> Subtract consecutive counts: `$n[1] - $b = $m` and `$n[2] - $n[1] = $m`. The same number each time, so the pattern grows by <b>adding `$m`</b> every minute. That subtraction is the test &mdash; dividing is the test for a <i>multiplying</i> pattern, and on this table it would not give the same answer twice.</p><p><b>Extending the table.</b> Keep adding `$m`: `$n[2] + $m = $t3` at `t=3`, and `$t3 + $m = $t4` at `t=4`.</p><p><b>The explicit rule.</b> The count starts at `$b` when `t=0` and gains `$m` per minute, so `B = $m t + $b`. In the picture, `$m` is the blocks added each minute and `$b` is the part that was there before the pattern started growing &mdash; it never moves.</p><p>Check it against a figure you can see: `B(2) = $m xx 2 + $b = $n[2]`, which matches the third picture.</p>"
