...
$solutionguide = "<p><b>The constant ratio.</b> Divide each count by the one before it: `$n[1] div $a = $r` and `$n[2] div $n[1] = $r`. Same number both times, so the constant ratio is `$r`.</p><p><b>The recursive rule</b> says where to start and what to do to get the next term: start at `$a`, multiply by `$r` each minute. Recursive rules are the quick way to the <i>next</i> term and a slow way to a far one: to reach `t=$far` you would multiply `$far` separate times.</p><p><b>The explicit rule</b> goes straight to any term. Starting value `$a`, multiplied by `$r` once per minute, gives `N = $a * $r^t`. The starting value is the multiplier out front and the constant ratio is the base of the exponent: not the other way round.</p><p><b>The far term.</b> `N($far) = $a * $r^$far = $farvalue`. This is the whole reason the explicit rule is worth writing down.</p>"
