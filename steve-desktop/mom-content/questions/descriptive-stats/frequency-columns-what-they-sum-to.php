...
<div class="sol-wrap" style="font-family:-apple-system,BlinkMacSystemFont,\'Segoe UI\',Arial,sans-serif; font-size:16px; line-height:1.6; color:#21242c; max-width:688px; margin:1em 0;">
  <details>
    ...
    <div class="sol-body">
      <p><span class="term-label">Part (a): the frequency column.</span> ' . $f[0] . ' + ' . $f[1] . ' + ' . $f[2] . ' + ' . $f[3] . ' + ' . $f[4] . ' = <b>' . $n . '</b>. Every ' . $unitOne . ' surveyed gave exactly one answer and is counted in exactly one row, so the column has to come back to the sample size. If it does not, a response has been missed or counted twice.</p>
      <p><span class="term-label">Part (b): the relative frequency column.</span> <b>1</b>. Each entry is that row\'s count over ' . $n . ', so adding them is the same as adding all the counts and then dividing once: ' . $n . ' / ' . $n . ' = 1. As percentages, 100%. Rounding each row separately can leave the printed column a tenth off, which is worth expecting rather than treating as an error.</p>
      <p><span class="term-label">Part (c): frequency against relative frequency.</span> For the value ' . $askValue . ', the frequency is ' . $askFreq . ': a headcount. The relative frequency is ' . $askFreq . ' / ' . $n . ' &approx; ' . $askRel . ', or ' . $askRelPct . '%: a share of the sample. The count answers "how many", the share answers "how much of the group", and only the share can be compared against a survey of a different size.</p>
      <p><span class="term-label">Part (d): relative against cumulative relative.</span> Relative frequency is that row on its own. Cumulative relative frequency runs from the first row through that one: for ' . $askValue . ' it is (' . $cumThrough . ') / ' . $n . ' &approx; ' . $askCum . ', so about ' . $askCumPct . '% answered ' . $askValue . ' or fewer. The last row\'s cumulative figure is always 1, which is a free check on the whole column.</p>
