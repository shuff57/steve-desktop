...
<div class="sol-wrap" style="font-family:-apple-system,BlinkMacSystemFont,\'Segoe UI\',Arial,sans-serif; font-size:16px; line-height:1.6; color:#21242c; max-width:688px; margin:1em 0;">
  <details>
    ...
    <div class="sol-body">
      <p><span class="term-label">Check 1: does the line ever come back down?</span> The frequencies are ' . $f[0] . ', ' . $f[1] . ', ' . $f[2] . ', ' . $f[3] . ', ' . $f[4] . ', so no point should sit above ' . $maxF . '. One graph finishes at ' . $lastCum . ': not a frequency at all, but the total number of people surveyed. It is plotting the running total up to each value, which is why it only ever climbs.</p>
      <p><span class="term-label">Check 2: where does the vertical axis start?</span> Read the bottom label on each axis, not the shape of the line. One graph starts its axis at ' . ($minF - 1) . ' rather than 0. The data is unchanged, but the smallest value is squashed to nothing and the gaps look far larger than they are.</p>
      <p><span class="term-label">Check 3: do the heights match the table?</span> Take one value and check it. The table gives ' . $f[0] . ' for the value 0; one graph shows ' . $f[2] . ' there instead, because its frequencies have been shifted along and attached to the wrong values. Nothing about that graph looks wrong on its own: the only way to catch it is to read it against the table.</p>
