// === NAME - DESCRIPTION: Hypothesis Test for the Slope - State H0/Ha, compute t, interpret p-value ===
// === SET QUESTION TYPE TO: multipart ===

// === COMMON CONTROL ===

$anstypes = array("choices", "choices", "number", "choices")
...
// Pick (target_t, p) pair so the output is self-consistent
...
$computed_t = round($slope_est / $slope_se, 2)
...
<div class="sol-wrap" style="font-family:-apple-system,BlinkMacSystemFont,\'Segoe UI\',Arial,sans-serif; font-size:16px; line-height:1.6; color:#21242c; max-width:688px; margin:1em 0;">
  <details>
    ...
    <div class="sol-body">
      ...
      <p><b>(c) t-statistic.</b> `t = b_1 / SE = ' . $slope_est . ' / ' . $slope_se . ' = ' . $computed_t . '`.</p>
      <p><b>(d) p-value decision.</b> The p-value `' . $slope_p . '` is ' . $compare_text . ', so we ' . $decision_text . '. ' . $sig_summary . '</p>
...
<div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif; font-size:16px; line-height:1.6; color:#21242c; max-width:688px;">
  ...
  <div style="background:#fff; border:1px solid #e5e7eb; border-radius:12px; padding:20px; margin:10px 0; box-shadow:0 4px 6px -1px rgba(0,0,0,0.07),0 2px 4px -2px rgba(0,0,0,0.04);">
    <span style="display:inline-block; background:#e8f0fe; color:#1865f2; border-radius:6px; padding:3px 10px; font-size:13px; font-weight:700; margin-right:10px; vertical-align:middle;">c.</span> Compute the t-statistic. Round to 2 decimal places.
    <div style="margin-top:12px;text-align:center;">$answerbox[2]</div>
  </div>
  <div style="background:#fff; border:1px solid #e5e7eb; border-radius:12px; padding:20px; margin:10px 0; box-shadow:0 4px 6px -1px rgba(0,0,0,0.07),0 2px 4px -2px rgba(0,0,0,0.04);">
    <span style="display:inline-block; background:#e8f0fe; color:#1865f2; border-radius:6px; padding:3px 10px; font-size:13px; font-weight:700; margin-right:10px; vertical-align:middle;">d.</span> Choose the best interpretation of the p-value at `alpha = 0.05`. $answerbox[3]
  </div>
</div>
