...
/**
 * The page-agent Re-Act system prompt, vendored from page-agent's source.
 * Drives the LLM through an observe → reflect → act loop with a MacroTool
 * output that enforces reflection-before-action.
 *
 * The prompt expects these input sections to be appended by the loop driver:
 *   <instructions>      — optional skill-specific rules (system + page-level)
 *   <agent_state>       — user_request + step_info
 *   <agent_history>     — chronological step events
 *   <browser_state>     — URL + indexed interactive elements + page content
 */
...
/**
 * Build the user-facing prompt with the dynamic sections the system prompt expects.
 * Mirrors PageAgentCore.#assembleUserPrompt from page-agent.
 */
export function assembleUserPrompt(opts: {
  ...
  prompt += '</user_request>\n';
  prompt += '<step_info>\n';
  prompt += `Step ${stepCount + 1} of ${maxSteps} max possible steps\n`;
...
export type HistoryEntry = AgentStepEvent | ObservationEvent;
